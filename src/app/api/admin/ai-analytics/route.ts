 ...
 * Provides comprehensive analytics data for AI interactions
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      getConversationQuality(startDate)
    ])
...
async function getSessionAnalytics(startDate: Date) {
  ...
  const { data: sessions } = await supabase
    .from('ai_sessions')
    .select('session_id, user_hash, message_count, created_at, last_activity')
...
async function getConversationMetrics(startDate: Date) {
  ...
  // Use nullish coalescing to safely handle undefined
  const avgUserMessageLength = conversations && conversations.length > 0
    ? conversations.reduce((sum, c) => sum + c.user_message.length, 0) / conversations.length
    : 0

  const avgAiResponseLength = conversations && conversations.length > 0
    ? conversations.reduce((sum, c) => sum + c.ai_response.length, 0) / conversations.length
    : 0
...
async function getConversationQuality(startDate: Date) {
  ...
  // FIX: Proper null safety check - this was the line 327 error
  // Instead of: conversations?.length || 0 ? conversations.flatMap(...)
  // We use: conversations && conversations.length > 0 ? conversations.flatMap(...)
  
  const avgUserMessageLength = sessions && sessions.length > 0
    ? sessions
        .flatMap((s: any) => s.messages?.filter((m: any) => m.role === 'user') || [])
        .map((m: any) => m.content?.length || 0)
        .reduce((sum: number, len: number) => sum + len, 0) / 
        (sessions.flatMap((s: any) => s.messages?.filter((m: any) => m.role === 'user') || []).length || 1)
    : 0

  const avgAiMessageLength = sessions && sessions.length > 0
    ? sessions
        .flatMap((s: any) => s.messages?.filter((m: any) => m.role === 'assistant') || [])
        .map((m: any) => m.content?.length || 0)
        .reduce((sum: number, len: number) => sum + len, 0) / 
        (sessions.flatMap((s: any) => s.messages?.filter((m: any) => m.role === 'assistant') || []).length || 1)
    : 0
    ...
    sessions.forEach((s: any) => {
      if (s.messages && Array.isArray(s.messages)) {
        for (let i = 1; i < s.messages.length; i++) {
          if (s.messages[i].role === 'assistant' && s.messages[i-1].role === 'user') {
            if (s.messages[i].timestamp && s.messages[i-1].timestamp) {
              const timeDiff = new Date(s.messages[i].timestamp).getTime() - 
                              new Date(s.messages[i-1].timestamp).getTime()
              responseTimes.push(timeDiff / 1000) // in seconds
            }
          }
        }
      }
  ...
  // Engagement metrics
  const totalSessions = sessions?.length || 0
  const sessionsWithMultipleExchanges = sessions?.filter((s: any) => 
    s.message_count >= 4 // At least 2 exchanges (user + assistant × 2)
  )?.length || 0
  
  const engagementRate = totalSessions > 0 
    ? (sessionsWithMultipleExchanges / totalSessions) * 100 
    : 0
